...
fn main() {
    ...
    let goblin_ap = 3;
    for elf_ap in goblin_ap.. {
        if let Some(outcome) = simulate(grid.clone(), units.clone(), elf_ap, goblin_ap) {
            println!("{}", outcome);
            break;
        }
    }
...
fn simulate(mut grid: Vec<Vec<char>>, mut units: Vec<Unit>, elf_ap: i32, goblin_ap: i32) -> Option<i32> {
